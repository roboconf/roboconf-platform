...
public class AgentMessageProcessor extends AbstractMessageProcessor {
	...
	private IAgentClient messagingClient, newMessagingClient;
	...
	boolean processMsgInstanceStart( MsgCmdInstanceStart msg ) throws IOException {
		...
		if( instance == null ) {
			...
			try {
				...
				this.messagingClient.sendMessageToTheDm( new MsgNotifInstanceChanged( this.agent.getApplicationName(), instance ));
	...
	throws IOException, PluginException {
		...
		if( haveAllImports ) {
			if( impactedInstance.getStatus() == InstanceStatus.STARTING ) {
				...
				this.messagingClient.sendMessageToTheDm( new MsgNotifInstanceChanged( this.agent.getApplicationName(), impactedInstance ));
